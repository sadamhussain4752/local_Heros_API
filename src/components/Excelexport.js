...
const ExportCSV = ({ csvData, fileName, wscols, headers, headerTitle,title }) => {
  const fileType =
  ...
  const exportToCSV = (csvData, fileName, wscols) => {
    ...
    // Add headers
    XLSX.utils.sheet_add_json(ws, Heading, { skipHeader: true, origin: "A2" });
  ...
  return (
    ...
    <div className="text-center mt-2 mb-2 text-md-end pe-5">
      <button
        ...
        onClick={(e) => exportToCSV(csvData, fileName, wscols)}
      >
        <i className="fa-solid fa-download" /> Export XLSX
      </button>
    </div>
  );
};
